...
EOF
  if [ -z "${TEST_SUITE_FOLDER:-}" ]; then
    echo -e "\nRunning all tests..."
    go run github.com/onsi/ginkgo/v2/ginkgo run --timeout 3h --grace-period 5m ./...
  else
    echo -e "\nRunning tests in ${TEST_SUITE_FOLDER}..."
    go run github.com/onsi/ginkgo/v2/ginkgo run --timeout 3h --grace-period 5m -r "$TEST_SUITE_FOLDER" 
  fi
popd >/dev/null
